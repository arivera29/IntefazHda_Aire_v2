...
{
    public partial class FrmDistribuir : Form
    {
        public FrmDistribuir()
        {
            InitializeComponent();
        }
        ...
        {
            if (MessageBox.Show("Esta seguro de distribuir las actas rechazadas?", "Confirmar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                if (conexion != null)
                {
                    ...
                    String sql = "Select _number From Actas with(nolock) Where EstadoActa=5";
                    ...
                    sql = "Select BandCodi From Bandejas Where BandEsta=1 And BandTiba=4";
                    using (SqlCommand cmd = new SqlCommand(sql)) {
                        cmd.Connection = conexion.getConection();
                        cmd.Prepare();
                    ...
                    foreach (Int32 acta in Actas)
                    {
                        ...
                        sql = "UPDATE Actas set Bandeja=@bandeja Where _number=@acta And EstadoActa=5";
                        using (SqlCommand cmd = new SqlCommand(sql))
                        {
                            cmd.Connection = conexion.getConection();
